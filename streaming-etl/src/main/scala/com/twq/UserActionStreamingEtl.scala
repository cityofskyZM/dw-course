...
object UserActionStreamingEtl {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("UserActionStreamingEtl").setMaster("local[2]")
    val outputPath = sparkConf.get("spark.user.action.outputPath",
      "hdfs://master:9999/user/hadoop-twq/dw-course/streaming-etl/user-action-parquet")
